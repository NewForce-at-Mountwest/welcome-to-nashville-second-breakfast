const searchMeetups = meetUpParam => {
  ...
  fetch(
    `https://www.eventbriteapi.com/v3/events/search/?q=nashville_${meetUpParam}&token=WZJG657L2ZKPAVFFGYPQ`,
    {
      headers: {
    ...
    .then(function(parsedMeetups) {
      ...
      parsedMeetups.events.slice(-4).forEach(event => {
        document.querySelector(
          "#resultsContainer"
        ).innerHTML += `<div class="individual-meetup"><h3>${
          event.name.text
        }</h3><button id="saveButton3">Save</button></div>`;
...
document.querySelector("#resultsContainer").addEventListener("click", () => {
  ...
  if (event.target.id === "saveButton3") {
    ...
    document
      .querySelector("#meetupItineraryContainer")
      .appendChild(itineraryMeetup);
