const searchParks = taco => {
  ...
  fetch(
    `https://data.nashville.gov/resource/xbru-cfzi.json?$$app_token=VYe8wkkThsk0EjGHsubsD5hTx&&${taco}=Yes`
  )
    .then(parks => parks.json())
    .then(parsedParks => {
      console.log(parsedParks);
      parsedParks.slice(-4).forEach(parkInfo => {
        parkHTML += `<div class="individual-park"><h3>${
          parkInfo.park_name
        }</h3><p>${
          parkInfo.mapped_location_address
        }</p><button id="save-button">Save</button></div>`;
      });
...
document.querySelector("#resultsContainer").addEventListener("click", () => {
  ...
  if (event.target.id === "save-button") {
    ...
    document
      .querySelector("#parkItineraryContainer")
      .appendChild(itineraryPark);
